...
public:
	...
	{
		return n1->data().second + n1->m_h > n2->data().second + n2->m_h;
	}
};
...
void Graph<NodeType, ArcType>::adaptedBreadthFirst( Node* current, Node *goal)
{
	...
	//Both ensure that the loop ends when the target is found or if the target is invalid
...
void Graph<NodeType, ArcType>::ucs(Node* start, Node* dest, std::function<void(Node *)> f_visit, std::vector<Node *> & path)
{
	...
	while (nodeQueue.size() != 0 && goalReached == false) {
		...
		for (; iter != endIter; iter++)
		{
			...
			// Path cost at the child
			auto pathCost = (*iter).node()->data().second;

			if ((*iter).node() != nodeQueue.top())
			{
				int dist = (*iter).weight() + pathCost;
...
void Graph<NodeType, ArcType>::aStar(Node* start, Node* dest, std::function<void(Node *)> f_visit, std::vector<Node *> & path)
{
	...
	for (Node * node : m_nodes)
	{
		if (node != start)
		{
			node->data().second = std::numeric_limits<int>::max() - 10000;
			// Set the heuristic
			node->m_h = sqrt((node->m_x - dest->m_x) * (node->m_x - dest->m_x) +
				(node->m_y - dest->m_y) * (node->m_y - dest->m_y));
	...
	while (nodeQueue.size() != 0 && nodeQueue.top() != dest)
	{
		...
		auto iter = nodeQueue.top()->arcList().begin();
		auto endIter = nodeQueue.top()->arcList().end();
		std::cout << "Expanding: " << nodeQueue.top()->data().first << std::endl;
		for (; iter != endIter; iter++)
		{
			auto pathCost = nodeQueue.top()->data().second;
			//std::cout << pathCost << std::endl;
			if ((*iter).node() != nodeQueue.top()->previous())
			{
				std::cout << "Visiting: " << (*iter).node()->data().first << std::endl;
				int dist = (*iter).weight() + pathCost + (*iter).node()->m_h;

				if (dist < (*iter).node()->data().second + (*iter).node()->m_h)
				{
					(*iter).node()->data().second = (*iter).weight() + pathCost;
					(*iter).node()->setPrevious(nodeQueue.top());
				}
				if ((*iter).node()->marked() == false)
				{
